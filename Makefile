...
#	@${PYBIN}cookiecutter -f --no-input --config-file ${INSTANCE_YAML} devsrc/cookiecutter-zope-instance
##############################################################################
# TESTING
...
LDAPRELEASE=2.4.59
BASE_DIR=.
OPENLDAP_BASE_DIR=${BASE_DIR}/.openldap
OPENLDAP_DOWNLOAD_DIR=${OPENLDAP_BASE_DIR}/downloads
OPENLDAP_ARCHIVE=${OPENLDAP_DOWNLOAD_DIR}/openldap-${LDAPRELEASE}.tgz
${OPENLDAP_ARCHIVE}:
	@echo "$(OK_COLOR)Download openldap-${LDAPRELEASE}.tgz to ${OPENLDAP_DOWNLOAD_DIR} as ${OPENLDAP_ARCHIVE}$(NO_COLOR)"
	@mkdir -p ${OPENLDAP_DOWNLOAD_DIR}
	@curl -o ${OPENLDAP_ARCHIVE} https://www.openldap.org/software/download/OpenLDAP/openldap-release/openldap-${LDAPRELEASE}.tgz

OPENLDAP_INSTALL_DIR=${OPENLDAP_BASE_DIR}/inst
OPENLDAP_BIN_DIR=${OPENLDAP_INSTALL_DIR}/bin
OPENLDAP_SLAPD=${OPENLDAP_INSTALL_DIR}/libexec/slapd
${OPENLDAP_SLAPD}: ${OPENLDAP_ARCHIVE}
	@echo "$(OK_COLOR)Unpack ${OPENLDAP_ARCHIVE}$(NO_COLOR)"
	@mkdir -p ${OPENLDAP_INSTALL_DIR}
	@tar xf ${OPENLDAP_ARCHIVE} -C ${OPENLDAP_BASE_DIR}
	@echo "$(OK_COLOR)Build OpenLDAP ${LDAPRELEASE}$(NO_COLOR)"
	@./build_ldap.sh ${OPENLDAP_BASE_DIR}/openldap-${LDAPRELEASE} ${shell realpath ${OPENLDAP_INSTALL_DIR}}

.PHONY: test
test: ${TEST_PREREQUISITES} ${TESTRUNNER_SENTINEL} ${OPENLDAP_SLAPD} ## run tests
	@echo "$(OK_COLOR)Run addon tests$(NO_COLOR)"
	@SLAPD_BIN=${OPENLDAP_SLAPD} LDAP_ADD_BIN=${OPENLDAP_BIN_DIR}/ldapadd LDAP_DELETE_BIN=${OPENLDAP_BIN_DIR}/ldapdelete SLAPD_URIS=ldap://127.0.0.1:12345 ${PYBIN}zope-testrunner --auto-color --auto-progress --test-path=${ADDONFOLDER}

.PHONY: test-ignore-warnings
test-ignore-warnings: ${TEST_PREREQUISITES} ${TESTRUNNER_SENTINEL} ${OPENLDAP_SLAPD} ## run tests (hide warnings)
	@echo "$(OK_COLOR)Run addon tests$(NO_COLOR)"
	@SLAPD_BIN=${OPENLDAP_SLAPD} LDAP_ADD_BIN=${OPENLDAP_BIN_DIR}/ldapadd LDAP_DELETE_BIN=${OPENLDAP_BIN_DIR}/ldapdelete SLAPD_URIS=ldap://127.0.0.1:12345 PYTHONWARNINGS=ignore ${PYBIN}zope-testrunner --auto-color --auto-progress --test-path=${ADDONFOLDER}
